...
<script>
	...
	export default {
		...
		methods: {
			...
			toInsureAppoint: function(index) {
				...
				let appointInfo = {
					...
					checkID: this.visitPlan.checkId,
					clinicId: this.visitPlan.clinicId
				}
				uni.setStorageSync('appointInfo', JSON.stringify(appointInfo))
				uni.navigateTo({
					url: '确认预约页面'
				})
			},
			// 获取医生的该天的出诊计划
			getCheckPlan: function() {
				uni.showLoading({
					title: '加载中'
				})
				Let returnCode = 200
				let date = formDate(this.checkPlan.day, 'YYYY-MM-DD');
				let that = this;
				getHosipitalCheckPlan(JSON.parse(uni.getStorageSync('hospital')).id, this.appointInfo.checkId, date).then(res => {
					if(res.data.code === returnCode) {
						uni.hideLoading()
						res.data.data.forEach(function(item) {
							if(that.checkPlan.time == item.time) {
								that.visitPlan = item
							}
						})
					}
		...
		created() {
		this.checkPlan = JSON.parse(uni.getStorageSync('appointInfo')).outCallList
		this.getCheckPlan()
		uni.removeStorageSync("appointInfo")
		}
	}
</script>
